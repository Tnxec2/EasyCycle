package com.kontranik.cycleora.ui.statistic
...
import com.kontranik.cycleora.database.sdfIso
import com.kontranik.cycleora.databinding.FragmentStatisticBinding
...
class StatisticFragment : Fragment() {
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        inflater.inflate(com.kontranik.cycleora.R.menu.statistic_toolbar_menu, menu)
    ...
    private fun importStatisticFromFile(fileUri: Uri?) {
        if ( fileUri != null) {
            try {
                ...
                Log.d("Cycleora", "Imported items $countSaved")
                if ( countSaved > 0) {
                    val lastCycle = SettingsService.loadLastCycleStart(requireContext())
                    if ( lastCycle == null) {
                        Log.d("Cycleora", "set lastCycle...")
